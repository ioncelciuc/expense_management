...
class _ReceiptCaptureScreenState extends State<ReceiptCaptureScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<ReceiptCaptureCubit, ReceiptCaptureState>(
      listener: (context, state) {
        if (state is ReceiptCaptureSuccess) {
          for (Map<String, dynamic> map in state.items) {
            ...
            try {
              ...
              logger.severe('Error on date format: $e');
              selectedDates.add(DateTime.now());
            }
            try {
              String purchaseTypeName = map['purchaseType'];
              PurchaseType selectedPt = widget.purchaseTypes.firstWhere(
                (p) => p.name == purchaseTypeName,
                orElse: () => widget.purchaseTypes.first,
              );
              selectedPurchaseTypes.add(selectedPt);
            } catch (e) {
              logger.severe('Error on purchase type: $e');
          ...
          Fluttertoast.showToast(
            msg: state.response.message ?? 'Unknown error',
            backgroundColor: Colors.red,
          );
        }
      },
      child: BlocBuilder<ReceiptCaptureCubit, ReceiptCaptureState>(
        builder: (context, state) {
          bool isLoading = state is ReceiptCaptureLoading;
...
- date (String, format should be dd/MM/yyyy. if no year/month is defined, take the year as ${DateTime.now().year} and the month as ${DateTime.now().month})
- purchaseType

The purchase type should be the most suitable one for that specific product from the following: ${widget.purchaseTypes.map((e) => e.name).toList()}.
...
''';
    ...
    if (mounted) {
      BlocProvider.of<ReceiptCaptureCubit>(context).processReceipt(prompt, base64Image);
    }
  }
}
